...
if ($member_id != '') {
    ...
    $select_query = "SELECT tap.*,
        ...
        MINUTE ) ) 
        AND TIME( '".date('Y-m-d H:i:s') ."' ) or `tap_time` >  TIME( '".date('Y-m-d H:i:s') ."' ) )  AND tap.admin_id = $member_id AND FIND_IN_SET($member_id , replace(user_group.members_id,'|',',')) > 0  order by tap_time "  ;
